...
export function startVideo(
  ...
  let outerPc : RTCPeerConnection | null = null;
  let outerInterval : NodeJS.Timeout | null = null
  ...
  clearMessages({ encounterId: encounterId, toRole: remoteRole, fromRole: localRole }).then(() => {
    getMediaStream().then((mediaStream) => {
      ...
      const interval = setInterval(readRemoteMessage, 2000);
  ...
  return () => {
    ...
    getMediaStream().then((m) => {m.getTracks().forEach((track) => {
        track.stop();
      });
    });
